...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        q_vals = sum(
            prob * (self.mdp.getReward(state, action, nextState) + self.discount * self.getValue(nextState)) for
            nextState, prob in transitions)
        return q_vals
...
class PrioritizedSweepingValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        "*** YOUR CODE HERE ***"
        all_states = self.mdp.getAllStates()
        ...
        for state in all_states:
            if not self.mdp.isTerminal(state):
                diff = abs(self.values[state] - self.mdp.computeQValue(state))
        ...
        for _ in range(self.iterations):
            if myHeap.isEmpty():
                ...
                state = myHeap.pop()
                if not self.mdp.isTerminal(state):
                    self.values[state] = self.mdp.computeQValues(state)
                for p in predecessors[state]:
                    if self.mdp.isTerminal(p):
                        diff = abs(self.values[p])
                    else:
                        diff = abs(self.values[p] - self.mdp.computeQValues(p))
