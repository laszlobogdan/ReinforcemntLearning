...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(all_actions) == 0:
            ...
            q_values = [self.getQValue(state, action) for action in all_actions]
            max_q_value = max(q_values)

            # Collect all actions with the same Q-value as the maximum
            possible_actions = [action for action in all_actions if self.getQValue(state, action) == max_q_value]
    ...
    def getAction(self, state):
        ...
        if util.flipCoin(self.epsilon):
            ...
            # Take the best policy action with probability 1 - self.epsilon
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        td_error = target - self.getQValue(state, action)

        feature_vector = self.featExtractor.getFeatures(state, action)
        for feature in feature_vector:
            self.weights[feature] += self.alpha * td_error * feature_vector[feature]
